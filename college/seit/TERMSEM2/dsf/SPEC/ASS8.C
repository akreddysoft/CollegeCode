/*REPRESENT SPARSE MATRIX USING ARRAY AND PERFORM
...
void main()
{
int mat[30][30],sp[10][3],spt[10][3],m,n,i,j,nonzero=0,k,l;
clrscr();
...
for(i=0;i<n;++i)    //n is imp
	for(j=1;j<k;++j)           //k no of rows-1 in sp=l
		if(sp[j][1]==i)
		{
			spt[l][0]=sp[j][1];
			spt[l][1]=sp[j][0];
