...
*					==========================            *
*	 SUBMITTED BY:  MADHUR AHUJA ROLL NO 22               *
**********************************************************/
#include<stdio.h>
#include<conio.h>
#include<dos.h>
...
while(another=='y')
{
			 ...
			 fflush(stdin);
			 scanf("%c",&e1.sex);
			 if(e1.sex!='m'&&e1.sex!='f'&&e1.sex!='M'&&e1.sex!='F')
			 {
				printf("\nInvalid Input");
				getch();
				break;
			 ...
			 fare=e.dist*0.4;  //find if train exists and cal fare
...
void status()
{
		...
		printf("                            ======================\n");
		printf("\nName\t\tTrain No\tPNR NO\t\tTicket No.\tDate\n");
		drawline();
		while(fread(&e1,sizeof(e1),1,fp1)==1)
		 {
			 flag=1;
			 printf("%s\t\t%d\t\t%d\t\t%d\t\t%d/%d/%d\n",e1.name,e1.tr_no,e1.pnr_no,e1.t_no,e1.day,e1.month,e1.year);
		 }
	  if(!flag)
	  printf("\nRecord not found");
	  getch();
