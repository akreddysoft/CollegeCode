...
//	CEdit spy_timer;
	CString tm;
	int tmi=atoi(tm);
	spy_timer.GetWindowText(tm);
	if(!strcmpi(tm,"")) {
		SpyAppln();
	//	GetProcList();
	}
	else {
	//	SetTimer(this,4456,tmi,(TIMERPROC)OnTimer);
		//AfxMessageBox("Timer Starts");
	SetTimer(4456,tmi,(TIMERPROC)NULL);
	}
...
int CSpyView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	spy_sc.Create(WS_CHILD|WS_VISIBLE,CRect(180,153,510,177),this,1123);
	spy_dc.Create(WS_CHILD|WS_VISIBLE,CRect(520,153,850,177),this,1124);
	spyLog.Create(LBS_HASSTRINGS|LBS_STANDARD|WS_CHILD|WS_VISIBLE,CRect(175,205,988,310),this,34561);
//	CEdit spy_timer;
	spy_timer.Create(WS_VISIBLE|WS_CHILD|WS_OVERLAPPED,CRect(860,153,990,177),this,2120);
