...
void CPing::Ping(UINT nRetries,LPCSTR pstrHost,HWND hWnd)
{
	...
	if (lpHost == NULL)
	{
		...
		::PostMessage(m_hWnd,WM_PING_END, 21, (LPARAM) AllocBuffer(host));
		return;
	}
	...
	for (nLoop = 0; nLoop < nRetries; nLoop++)
	{
		...
		{
			str.Format(host);
			::PostMessage(m_hWnd,WM_MSG_STATUS, 23, (LPARAM) AllocBuffer(str));
		}
		else

		{
			...
			str.Format("Reply[%d] from: %s: bytes=%d time=%ldms TTL=%d", 
				nLoop+1,
				   ...
				   dwElapsed,
		           cTTL);
				//AfxMessageBox(pstrHost);
			::PostMessage(m_hWnd,WM_MSG_STATUS, 42, (LPARAM) AllocBuffer(pstrHost));

			Sleep(1000);
		}
	}
	
	::PostMessage(m_hWnd,WM_PING_END, 55, (LPARAM)AllocBuffer(pstrHost));
...
{
	CString strMsg;
	if(!strcmpi(lpMsg,"sendto()")) {
		strMsg.Format(host);
		::PostMessage(m_hWnd,WM_MSG_STATUS, 20, (LPARAM) AllocBuffer(strMsg));
	}
	else {
