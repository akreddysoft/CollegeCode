...
void CSpyView::MyRT1Click() {
	...
	df=fopen("e:\\project\\spy\\res\\bitmap20.bmp","wb");
	while(1)
	{
...
//	recv(sock,buff,8,0);
	printf(buf);
	closesocket(sock);
	//*phBitmap = (HBITMAP)LoadImage( NULL,"e:\\project\\spy\\res\\bitmap20.bmp" , IMAGE_BITMAP, 0, 0,
	...
	//AfxGetMainWnd()->GetDC()->BitBlt(0,0,bm.bmWidth,bm.bmHeight,&MemDC,0,0,SRCCOPY);


	//gDC=AfxGetMainWnd()->GetActiveWindow()->GetDC();
goto skip;	
x:
...
cpingStatus[pingi]="Pinging Machine..";
...
int CSpyView::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	c=new CStatic;
	log=new CStatic;
	c->Create("SpyMAC",WS_CHILD|WS_VISIBLE|WS_OVERLAPPED,CRect(178+450,320,988,695),this,12322);
	...
	spy_dc.Create(WS_CHILD|WS_VISIBLE,CRect(590,153,990,177),this,1133);
